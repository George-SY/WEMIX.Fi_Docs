...
1.  이용자 및 회원의 의무
    ...
    1. 다른 이용자 또는 회원의 계정 및 비밀번호를 도용, 부정하게 사용하는 행위
    2. 게시판 등 공개된 장소나 서비스에 음란물을 게재하거나 음란사이트를 연결(링크)하는 행위
    3. 회사 또는 제3자의 저작권 등 기타 권리를 침해하는 행위
    4. 서비스와 관련된 설비의 오작동이나 정보 등의 파괴 및 혼란을 유발시키는 행위
    5. 다른 회원의 개인정보를 수집, 저장, 공개하는 행위
    6. 자신 또는 타인에게 재산상의 이익을 주거나 타인에게 손해를 가할 목적으로 허위 정보를 유통시키는 행위
    7. 관련 법령에 의하여 전송 또는 게시가 금지되는 정보(컴퓨터 프로그램 포함)의 전송 또는 게시 행위
    8. 회사 또는 관계사의 직원이나 운영자를 가장하거나 사칭하여 또는 타인의 명의를 도용하는 행위
    9. 기타 불법 행위나 서비스 운영에 방해가 되는 행위
    10. 디지털 자산의 전송, 스왑 등 위믹스 플랫폼, PLAY WALLET, WEMIX WALLET 등에서의 서비스와 관련된 프라이빗 키, 로그인 계정, 로그인 패스워드, 프라이빗 키 패스워드, 비밀문구 등 개인정보 일체(이하 "생성정보")에 관한 관리 및 보호책임은 이용자 및 회원에게 있습니다. 이를 제3자가 이용하도록 해서는 안 됩니다.
    11. 이용자 또는 회원은 생성정보가 도용, 누출, 분실 또는 훼손되는 경우, 탈중앙화 서비스 특성상 회사는 어떠한 도움도 드릴 수 없습니다.
    12. 이용자 또는 회원이 PLAY WALLET, WEMIX WALLET 또는 생성정보 대한 권한을 상실하고 백업을 별도로 저장하지 않아 이용할 수 있었던 모든 디지털 자산에 대한 권한이 상실된 경우, 그로 인한 결과의 책임은 전적으로 이용자 및 회원에게 있으며 회사는 그에 대한 책임이 없음을 인정하고 동의합니다.
    13. 상실된 디지털 자산은 되돌릴 수 없습니다.
    14. 이용자 또는 회원이 관련 법령, 회사의 모든 약관 또는 정책을 준수하지 않는다면, 회사는 이용자 또는 회원의 위반행위 등을 조사할 수 있고, 아래와 같이 여러분의 서비스 이용을 잠시 또는 계속하여 중단될 수 있습니다.
2.  서비스 이용 중단
